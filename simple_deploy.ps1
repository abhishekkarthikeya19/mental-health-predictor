...
$setupScript | Out-File -FilePath "$DEPLOY_DIR\setup.sh" -Encoding ASCII

# Make sure the setup script has Unix line endings
(Get-Content "$DEPLOY_DIR\setup.sh") | ForEach-Object { $_ -replace "`r`n", "`n" } | Set-Content "$DEPLOY_DIR\setup.sh" -NoNewline

# Compress the deployment directory
Write-Host "Compressing project files..."
$DEPLOY_ARCHIVE = "mental-health-predictor-deploy.zip"
...
rm -rf mental-health-predictor
mkdir -p mental-health-predictor
unzip -o mental-health-predictor-deploy.zip -d mental-health-predictor
